...
app.get('/todosTaskCount', async (req, res) => {
    console.log("todosTaskCount called on backend")
    totalTaskCount = await db.collection('todos').countDocuments({});
    // res.send(totalTaskCount)
    res.json(totalTaskCount)
})

app.get('/todosTaskCompletedCount', async (req, res) => {
    console.log("todosTaskCompletedCount called on backend")
    totalTasksCompletedCount = await db.collection('todos').countDocuments({done: true});
    res.json(totalTasksCompletedCount)
})
...
app.post('/todos/standardTemplate', async (req, res) => {
    ...
    await db.collection('todos').insertMany([
        {done: true, desc: 'Complete Application', due: '01/01/2021', tag: 'Academics'},
        {done: true, desc: 'Complete 2 Endpoints for Side Project', due: '05/01/2021', tag: 'Career'},
        {done: false, desc: 'Invest in Tech Stocks Via Sarwa / Wahed', due: '09/01/2021', tag: 'Finance'},
        {done: false, desc: 'Community Involvement Volunteering', due: '03/02/2021', tag: 'Community'},
        {done: false, desc: 'Laundry', due: '01/01/2021', tag: 'Home'},
        {done: false, desc: 'Electricity + Phone Bills', due: '03/04/2021', tag: 'Home'},
        {done: false, desc: 'Computer Science Exam Prep', due: '01/07/2021', tag: 'Academics'}
    ])
